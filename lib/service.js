...
module.exports = function injectToService(checks) {
  ...
  proto.listen = async function listen(eventNames, handler) {
    if (arguments.length !== 2) throw new Error('"listen" function expects two arguments');
    if (typeof eventNames !== 'string' && !Array.isArray(eventNames)) {
      throw new Error('"listen" first argument should be a string or array');
    }
    if (typeof handler !== 'function') throw new Error('"listen" second argument should be a function');
  ...
  proto.ask = async function ask(eventName, payload, metadata={}) {
    if (arguments.length < 2 || arguments.length > 3) throw new Error('"ask" function expects two or three arguments');
    if (typeof eventName !== 'string') throw new Error('"ask" first argument (event name) should be a string');
    if (!isObject(payload)) throw new Error('"ask" second argument (event payload) should be an object');
    if (!(metadata instanceof Object) || Array.isArray(metadata) || metadata === null) {
  ...
  proto.reply = async function reply(originalEvent, replyPayload, replyMetadata={}) {
    if (arguments.length < 2 || arguments.length > 3) throw new Error('"reply" function expects two or three arguments');
    if (!isReceivedEvent(originalEvent)) throw new Error('"reply" first argument should be the received event');
    if (!isObject(replyPayload)) throw new Error('"reply" second argument (event body) should be an object');
    if (!isObject(replyMetadata)) throw new Error('"reply" third argument (event payload) should be an object');
  ...
  proto.publish = async function publish(eventName, payload, metadata={}) {
    if (arguments.length < 2 || arguments.length > 3) throw new Error('"publish" function expects two or three arguments');
    if (typeof eventName !== 'string') throw new Error('"publish" first argument (event name) should be a string');
    if (!isObject(payload)) throw new Error('"publish" second argument (event payload) should be an object');
    if (!(metadata instanceof Object) || Array.isArray(metadata) || metadata === null) {
